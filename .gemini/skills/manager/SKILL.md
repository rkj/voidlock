...
2. **TOOL POLICY**: Use ONLY the `br` command (Beads skill) for issue management. `beads` is denied by policy.
...
1. Run `br list --status in_progress --json` to check for unfinished work.
2. If empty, run `br ready --sort priority --json -n 1` to pick the next task.

## 2. Task Delegation (The Dispatch)

**Action**: Spawn the `executor` subagent for implementation. DO NOT MODIFY FILES YOURSELF.

- **Status**: Run `br update <TASK_ID> --status in_progress` immediately.
- **Context**: Use `br comments add` to provide any missing specs or error logs.
...
- **IF VERIFIED**:
  ...
  2. Close the task with `br close <TASK_ID> --reason "<PROOF_OF_VERIFICATION>"`.
- **IF FAILED**:
  1. DO NOT close or revert (unless harmful).
  2. Log reason via `br comments add <TASK_ID> "FAILED_VERIFICATION: <REASON>"`.
