...
1. **OUTCOME-BASED AUDIT (CRITICAL)**: You are not an administrator; you are an **Auditor**. Do not accept a sub-agent's summary as proof of completion. You MUST verify the outcome yourself using DevTools/Screenshots before closing a task.
...
1. **TDD ENFORCEMENT**:
   - **Negative Proof**: Every bug fix MUST be preceded by a failing test (Unit or E2E). If the agent didn't provide one, **FAIL** the verification.
...
1. **Visual Audit (MANDATORY for UI/CSS/Layout)**:
   ...
   - Take screenshots at **1024x768** and **400x800**.
   - Compare screenshots against the **Product Spec** and the "Negative Proof" screenshots from the planning phase.
   - If the visual state is incorrect (e.g. still all-caps, still overflowing), **REJECT** the task.
...
- **If Verified**:
  1. `jj commit -m "<bead-id>: <description>.\n\nVerified visually at 1024x768 and 400x800."`
  2. `bd close <id> --reason "Implemented via sub-agent and verified via manual visual audit."`
