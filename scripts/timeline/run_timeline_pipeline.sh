...
if [[ -n "$PLAYBOOK_AGENT_CMD" ]]; then
  npm run timeline:playbooks -- --topology "$TOPOLOGY" --navigation-map "$NAV_MAP" --playbooks "$PLAYBOOKS" --provider "$PLAYBOOK_PROVIDER" --execute "$PLAYBOOK_EXECUTE" --agent-cmd "$PLAYBOOK_AGENT_CMD"
else
  npm run timeline:playbooks -- --topology "$TOPOLOGY" --navigation-map "$NAV_MAP" --playbooks "$PLAYBOOKS" --provider "$PLAYBOOK_PROVIDER" --execute "$PLAYBOOK_EXECUTE"
fi

echo "[timeline] capture screenshots"
npm run timeline:capture -- --manifest "$MANIFEST" --screenshots "$SCREENSHOTS" --port "$PORT" --max-count "$MAX_COUNT" --navigation-map "$NAV_MAP" --playbooks "$PLAYBOOKS"
