...
allTestFiles.forEach(filePath => {
  ...
  // Add clearCampaign to ConfigManager mocks
  if (content.includes('ConfigManager') && content.includes('vi.mock')) {
      if (content.includes('loadGlobal:') && !content.includes('clearCampaign:')) {
          content = content.replace(/loadGlobal: vi\.fn\(\)/, 'loadGlobal: vi.fn(),\n    clearCampaign: vi.fn()');
          changed = true;
      }
  }
