...
This guarantees that the Header and Footer are *always* visible, and only the Content area scrolls.
...
2.  **Phase 2 (Migration):** Port screens one by one.
    -   *Priority:* `EquipmentScreen`, `MissionSetupScreen` (Fixes the immediate bugs).
    -   *Secondary:* `MainMenu`, `CampaignScreen`.
