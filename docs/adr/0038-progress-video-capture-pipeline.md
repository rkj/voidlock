...
2. Keep screen labels correct (for example, mission screenshot must actually be mission).
3. Stitch the resulting frames into a timeline video.
...
2. **Capture workflow (deterministic executor)**
...
2. Targeted override support (single-commit fix without changing neighbors).
3. Reuse of previously validated plans without re-invoking the agent.
4. Faster reruns (capture consumes one ready DB, no plan inference at runtime).
...
2. Navigate to `/`.
3. Execute steps for target screen.
4. Scroll target into view before click attempts.
5. Verify target screen state in-browser.
...
2. Use existing `timeline/commit_playbooks.jsonl` as baseline.
3. Identify failing commit(s) from capture logs.
4. Patch only the relevant change-point or commit-level plan.
5. Recompile plan DB.
6. Rerun capture from failed commit onward.
7. Keep previous successful screenshots and timeline artifacts.
