...
1. Runtime state leaks (LocalStorage/session state) can alter startup screens and invalidate click paths.
1. Capture quality gates can produce false positives (wrong screen accepted) or false negatives (valid mission rejected as dark).
...
1. **Deterministic executor performs all screenshots** for all commits.
1. **Verification is mandatory** and blocks promotion of bad plans.
1. **Mission is hard-required by default** (except explicit allowlist entries).
...
1. Navigate to `/`.
1. Clear origin storage before flow.
1. Execute commit playbook steps (scroll into view before clicks).
...
1. Risky actions are blocked globally in executor.
1. Browser storage is cleared before each flow.
1. Any semantic mismatch (for example campaign accepted as mission) is treated as a pipeline defect and must be fixed in verifier/executor logic.
...
1. Regenerate `navigation_map` and `topology`.
1. Regenerate playbooks with constrained agent.
1. Rebuild `commit_playbooks.jsonl`.
1. Re-run capture with mission-required policy.
1. Add only truly broken SHAs to allowlist.
1. Re-run frame analysis and render.
