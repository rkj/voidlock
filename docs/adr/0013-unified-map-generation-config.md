...
1. Difficulty in passing a "map recipe" or configuration object between systems (e.g., from UI to Worker).
1. Fragile method signatures that grow with new optional parameters.
...
1. **Constructor Injection**: The `MapGenerator` class will accept this configuration in its constructor.
...
1. **Immutability**: The configuration is considered immutable for the lifespan of that `MapGenerator` instance. To generate a different map, instantiate a new generator.
