...
2.  **Validation:** checking if a generated map meets gameplay requirements (e.g., connectivity, spawn point counts).
3.  **Sanitization:** cleaning up artifacts (e.g., removing unreachable void cells, ensuring boundary consistency).
...
1.  **`MapFactory` (formerly `MapGenerator`):**
    -   Responsible *only* for accepting a `MapGenerationConfig` and returning a `MapDefinition`.
