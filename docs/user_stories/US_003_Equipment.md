...
1. Observe the "Campaign Context Header" -> Displays Difficulty, Mission Number, and Sector.
1. Click on a Deployment Slot to focus a soldier -> Soldier Inspector opens (within the Setup screen).
1. Click the "Engineering" or "Armory" tab/section -> List of weapons and items appears.
1. Click a "Pulse Rifle" (if not owned) -> Scrap is deducted, item is equipped to Right Hand.
1. Observe Weapon Stats (Damage, Range, Fire Rate) -> Displayed as icons.
1. Click "Launch Mission" -> Transition to tactical gameplay.
