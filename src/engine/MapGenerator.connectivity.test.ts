                ...
                for (const d of dirs) {
                    ...
                    if (b && !b.isWall) {
                        ...
                        if (nx < 0 || nx >= map.width || ny < 0 || ny >= map.height) {
                            expect.fail(`${genName} seed ${seed}: Cell ${x},${y} is open ${d} to map edge (Void)`);
                        }
                        const neighbor = graph.cells[ny][nx];
                        expect(neighbor.type, `${genName} seed ${seed}: Cell ${x},${y} is open ${d} to Void cell ${nx},${ny}`).toBe(CellType.Floor);
                    }
                }
            }
        }
