...
function computeReachable(graph: Graph, map: MapDefinition): Set<string> {
  ...
  while (head < queue.length) {
    ...
    for (const { dx, dy, d } of DIRS) {
      ...
      const isTraversable = boundary.type === BoundaryType.Open || boundary.type === BoundaryType.Door;
...
function rebuildWalls(graph: Graph, reachable: Set<string>): { walls: WallDefinition[]; boundaries: BoundaryDefinition[] } {
  ...
  for (const b of graph.getAllBoundaries()) {
    ...
    if (isC1Reachable !== isC2Reachable) shouldBeWall = true;
  ...
  return { walls: newWalls, boundaries: newBoundaries };
}
...
export class MapSanitizer {
  public static sanitize(map: MapDefinition): void {
    ...
    const reachable = computeReachable(graph, map);
    ...
    const { walls: newWalls, boundaries: newBoundaries } = rebuildWalls(graph, reachable);
    map.walls = newWalls;
    if (map.boundaries) {
    ...
    if (map.doors) {
      map.doors = map.doors.filter((door) => {
        ...
        return reachable.has(`${s1.x},${s1.y}`) && reachable.has(`${s2.x},${s2.y}`);
      });
    }
  }
}
