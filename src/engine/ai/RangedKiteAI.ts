...
export class RangedKiteAI implements IEnemyAI {
    think(enemy: Enemy, state: GameState, grid: Grid, pathfinder: Pathfinder, los: LineOfSight, prng: PRNG): void {
        ...
        if (targetSoldier) {
            ...
            if (dist < fleeThreshold) {
                ...
                const fleeTarget = this.findFleeTarget(enemy.pos, (targetSoldier as Unit).pos, grid, pathfinder);
                if (fleeTarget) {
    ...
    private findFleeTarget(start: Vector2, threat: Vector2, grid: Grid, pathfinder: Pathfinder): Vector2 | null {
        ...
        for (const c of candidates) {
            ...
            if (grid.isWalkable(cx, cy)) {
                ...
                if (dist > maxDist) {
                    // Ensure reachable
                    const path = pathfinder.findPath({x: Math.floor(start.x), y: Math.floor(start.y)}, {x: cx, y: cy});
                    if (path) {
                        maxDist = dist;
