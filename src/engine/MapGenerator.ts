import { MapDefinition, CellType, SpawnPoint, Objective, Cell, IMapValidationResult, Door, Vector2, TileAssembly, TileDefinition, MapGeneratorType, ObjectiveDefinition, WallDefinition } from '../shared/types';
...
export class MapGenerator {
  ...
  public static sanitize(map: MapDefinition): void {
      ...
      for (const b of graph.getAllBoundaries()) {
          ...
          if (!c1 || !c2 || c1.type === CellType.Wall || c2.type === CellType.Wall) {
              isWall = true;
          }
  ...
  public validate(map: MapDefinition): IMapValidationResult {
    ...
    if (spawnPoints && spawnPoints.length > 0) {
        ...
        for (const cellDef of map.cells) {
            if (cellDef.type === CellType.Floor && !visitedCells.has(`${cellDef.x},${cellDef.y}`)) {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        if (cell.type === CellType.Wall) {
          ...
          let cellChar = ' ';
          if (spawnPoints?.some(sp => sp.pos.x === x && sp.pos.y === y)) cellChar = 'S';
          else if (extraction && extraction.x === x && extraction.y === y) cellChar = 'E';
          else if (objectives?.some(obj => obj.targetCell?.x === x && obj.targetCell?.y === y)) cellChar = 'O';
  ...
  public static assemble(assembly: TileAssembly, library: Record<string, TileDefinition>): MapDefinition {
    ...
    const cells: Cell[] = Array(width * height).fill(null).map((_, i) => ({
        x: i % width, y: Math.floor(i / width), type: CellType.Wall
    }));

    const openBoundaries = new Set<string>();
    const getBoundaryKey = (x1: number, y1: number, x2: number, y2: number) => [`${x1},${y1}`, `${x2},${y2}`].sort().join('--');
    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            const neighbors = [{nx: x+1, ny: y}, {nx: x, ny: y+1}];
    ...
    const doors: Door[] = assembly.globalDoors?.map(d => ({
        id: d.id, orientation: d.orientation, state: 'Closed', hp: 50, maxHp: 50, openDuration: 1,
    ...
    return { width, height, cells, walls, doors,
        spawnPoints: assembly.globalSpawnPoints?.map(sp => ({ id: sp.id, pos: { x: sp.cell.x - minX, y: sp.cell.y - minY }, radius: 1 })) || [],
        extraction: assembly.globalExtraction ? { x: assembly.globalExtraction.cell.x - minX, y: assembly.globalExtraction.cell.y - minY } : undefined,
        objectives: assembly.globalObjectives?.map(obj => ({ id: obj.id, kind: obj.kind, state: 'Pending' as const, targetCell: { x: obj.cell.x - minX, y: obj.cell.y - minY } })) || []
    };
  }
  ...
  public static fromAscii(asciiMap: string): MapDefinition {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        cells.push({ x, y, type: char === '#' ? CellType.Wall : CellType.Floor });
        ...
        if (x < width - 1) {
            ...
            if (wallChar === '|') walls.push({ p1: {x, y}, p2: {x: x+1, y} });
            else if (wallChar === 'I') doors.push({ id: `d-${doors.length}`, orientation: 'Vertical', state: 'Closed', hp: 50, maxHp: 50, openDuration: 1, segment: [{x, y}, {x: x+1, y}] });
        }
        if (y < height - 1) {
            const wallChar = lines[ey + 1][ex];
            if (wallChar === '-') walls.push({ p1: {x, y}, p2: {x, y: y+1} });
            else if (wallChar === '=') doors.push({ id: `d-${doors.length}`, orientation: 'Horizontal', state: 'Closed', hp: 50, maxHp: 50, openDuration: 1, segment: [{x, y}, {x, y: y+1}] });
        }
      }
    }
    return { width, height, cells, walls, doors, spawnPoints, extraction, objectives };
  }
}
