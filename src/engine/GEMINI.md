...
- `GameClient.ts`: Provides an interface for the renderer (main thread) to communicate with the engine (worker). Exposes typed methods for debug actions (overlays, state queries) and handles time scaling, including Active Pause (0.05x). Now implements mission auto-save for crash recovery, persisting both the command log and the current engine tick to `localStorage`. Optimized for Replay mode by enforcing absolute pause (0.0x) and utilizing snapshots for fast-forwarding during seeks.
