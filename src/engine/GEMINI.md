...
- `CoreEngine.ts`: The main orchestrator of the game simulation. It manages state (including `isPaused`, `timeScale`, and `isSlowMotion`), initializes managers, and runs the game loop. It respects `allowTacticalPause`, enforcing minimum speed of 1.0x (except for absolute pause) when disabled. Now supports node-type specific mission setup (e.g., Boss/Elite nodes) and includes a catch-up phase for session recovery that utilizes state snapshots for near-instant fast-forwarding. Dynamically calculates the Director's `startingPoints` based on `baseEnemyCount`, `missionDepth`, and `enemyGrowthPerMission` to ensure balanced enemy scaling across campaign missions, specifically enforcing 0 initial roaming enemies for Mission 1 (depth 0). **Optimization:** State serialization now omits large static map data (cells, walls) after the first send, but always includes critical mission entities (spawnPoints, extraction, squadSpawns, objectives) to ensure authoritative visibility (ADR 0032). Implements visibility pruning in `getState(true)`, removing enemies, turrets, mines, and loot that are not currently visible/discovered by the squad unless `debugOverlayEnabled` is active. Uses a bitset for visibility/discovery to reduce GC pressure. **Fix:** Hydration from snapshots now restores authoritative map data if it was omitted in the snapshot, ensuring correct rendering during replay seeking.
...
- `GameClient.ts`: Provides an interface for the renderer (main thread) to communicate with the engine (worker). Exposes typed methods for debug actions (overlays, state queries) and handles time scaling, including Active Pause (0.1x). Now implements mission auto-save for crash recovery, persisting both the command log and the current engine tick to `localStorage`. Optimized for Replay mode by enforcing absolute pause (0.0x) and utilizing snapshots for fast-forwarding during seeks.
...
- `Graph.ts`: Represents the map as a graph of cells and boundaries (walls/doors). Supports sparse initialization from `MapDefinition` and authoritative hydration from `boundaries` array. Hydrates `walls` from corner-to-corner geometric segments. Now automatically treats boundaries between `Floor` and `Void` cells as `Wall` to ensure correct Line of Sight.
