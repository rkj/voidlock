...
- `ItemDistributionService.ts`: Handles opportunistic pickup logic and item assignments to units. Manages a spatial grid of visible loot and objectives to resolve competition between capable units.
...
- `MissionManager.ts`: Handles mission-specific setup, objective tracking, scrap reward calculation, and win/loss conditions. Supports special logic for `Boss` nodes (3 objectives, 3x scrap) and `Elite` nodes (2 objectives, 2x scrap), including specialized Hive/Recover mix. Correctly handles `isRecoverMission` by excluding `MissionType.Default` when `nodeType` is present, ensuring map-based Recover objectives become optional loot in campaign combat nodes while maintaining legacy test support. Mission completion (Won/Lost) is deferred until all units are either extracted or dead, ensuring a complete squad accounting before returning to the strategic layer. Uses `PlacementValidator` to ensure dynamic objectives do not overlap with existing map occupants.
