...
- `MovementManager.ts`: Translates path data into position updates and handles door interactions. Refactored to handle any `IMovableEntity` (Units or Enemies) via a generic interface. Now maintains a `positionHistory` ring buffer (last 6 cells) for units to support anti-backtracking AI rules (Spec 2.0).
