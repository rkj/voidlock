...
export class UseItemHandler implements IUnitCommandHandler {
  ...
  public execute({ unit, cmd, state, isManual, registry, director }: CommandExecParams): Unit {
    ...
    if (isTimedAction) {
      ...
      const scaledDuration = baseTime * (SPEED_NORMALIZATION_CONST / currentUnit.stats.speed);
      ...
      // Instant use
      currentUnit = this.applyInstantUse(currentUnit, state, useItemCmd, director);
    }

    return currentUnit;
  }

  private applyInstantUse(
    unit: Unit,
    state: GameState,
    ...
    if (count > 0) {
      ...
      if (director) {
        ...
        if (mutated) currentUnit = { ...currentUnit, hp: mutated.hp };
      }
    }
