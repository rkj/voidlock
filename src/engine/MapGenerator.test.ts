...
describe('MapGenerator.validate', () => {
  ...
  it('should validate map if a door provides reachability', () => {
    ...
    map.walls = [];
    map.doors = [
        { id: 'd1', segment: [{x: 0, y: 0}, {x: 1, y: 0}], orientation: 'Vertical', state: 'Closed', hp: 100, maxHp: 100, openDuration: 1 }
    ];
    const result = generator.validate(map);
    expect(result.isValid).toBe(true);
  });
  ...
  it('should invalidate map if a LOCKED door is the only path', () => {
    ...
    map.doors = [
        { id: 'd1', segment: [{x: 0, y: 0}, {x: 1, y: 0}], orientation: 'Vertical', state: 'Locked', hp: 100, maxHp: 100, openDuration: 1 }
    ];
    const result = generator.validate(map);
    expect(result.isValid).toBe(false);
    expect(result.issues).toContain('Floor cell at (1, 0) is not reachable from any spawn point.');
  });
});