...
describe("Regression mqdp: VIP Death Mission Success", () => {
  ...
  it("should fail even if other objectives are completed but VIP dies", () => {
    ...
    const engine = new CoreEngine(mapWithObjective, 123, squadConfig, true, false, MissionType.EscortVIP);
    const internalState = (engine as any).state;
    
    // Complete the other objective
    internalState.objectives.find((o: any) => o.id === "obj-1").state = "Completed";
  ...
  it("should fail if VIP dies in a Default mission (manually added VIP)", () => {
    ...
    const engine2 = new CoreEngine(mockMap, 123, squadWithVip, true, false, MissionType.Default);
    const internalState2 = (engine2 as any).state;
    
    expect(internalState2.objectives.some(o => o.id === "obj-escort")).toBe(true);
    
    // Kill the VIP
    const internalVip = internalState2.units.find((u: any) => u.archetypeId === "vip");
    internalVip.hp = 0;
    
    // Extract the assault
    const assault = internalState2.units.find((u: any) => u.archetypeId === "assault");
  ...
  it("should fail if VIP dies in a Default mission with no other objectives (The Bug)", () => {
    ...
    const engine = new CoreEngine(mapNoObj, 123, squadWithVip, true, false, MissionType.Default);
    const internalState = (engine as any).state;
    
    // In Default mission, setupMission adds obj-escort if hasVipInSquad is true.
    expect(internalState.objectives.some(o => o.id === "obj-escort")).toBe(true);
    ...
    // Extract the assault
    const assault = internalState.units.find((u: any) => u.archetypeId === "assault");
    assault.state = UnitState.Extracted;
  ...
  it("should fail if VIP dies in an ExtractArtifacts mission", () => {
    ...
    const engine = new CoreEngine(artifactMap, 1, squadWithVip, true, false, MissionType.ExtractArtifacts);
    const internalState = (engine as any).state;
    
    // Verify artifacts are present and obj-escort is present
    expect(internalState.objectives.length).toBeGreaterThan(0);
    expect(internalState.objectives.some((o: any) => o.kind === "Escort")).toBe(true);
    expect(internalState.objectives.some((o: any) => o.kind === "Recover")).toBe(true);
