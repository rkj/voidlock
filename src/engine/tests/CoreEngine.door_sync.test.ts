...
describe('CoreEngine Door Sync', () => {
    ...
    doors: [
      {
        ...
        state: 'Closed',
        hp: 100, maxHp: 100, openDuration: 0.5 // 500ms
      }
    ],
    spawnPoints: [],
    objectives: []
  ...
  it('should update Graph boundary isWall when door opens', () => {
    const engine = new CoreEngine(mockMap, 123, [], false, false, MissionType.Default);
    ...
    engine.addUnit({
        ...
        hp: 10, maxHp: 10,
        state: 'Idle' as any,
        damage: 1, fireRate: 100, attackRange: 5, sightRange: 5, speed: 1, aiEnabled: true, commandQueue: []
    });
  ...
  it('should update Graph boundary isWall when door closes', () => {
    ...
    const engine = new CoreEngine(mapWithOpenDoor, 123, [], false, false, MissionType.Default);
    const grid = (engine as any).gameGrid;
    const graph = grid.getGraph();
    const boundary = graph.getBoundary(0, 0, 1, 0);
