...
async function runSimulation() {
    ...
    for (let i = 0; i < ITERATIONS; i++) {
        ...
        const engine = new CoreEngine(map, seed, squad, true, false, MissionType.Default);
    ...
    console.log(`Avg Casualties (Wins): ${(wins > 0 ? totalCasualties/wins : 0).toFixed(2)}`);
    console.log(`Wins with Casualties: ${winsWithCasualties} (${(wins > 0 ? winsWithCasualties/wins*100 : 0).toFixed(1)}%)`);
}
