...
- `ConfigManager.ts`: Manages persistent game configuration and defaults (Map size, Unit Style, Mission types) in LocalStorage. Now includes strict validation and default fallback logic (Spec 8.12). Default mission generator is now set to `DenseShip` and default squad configuration now includes 4 soldiers to ensure consistency with the maximum squad size.
...
- `InputManager.ts`: Manages tactical input context, including keyboard shortcuts and mouse/touch interactions. Refactored to use `InputManagerConfig` for constructor parameters (ADR 0052). Implements 1-finger panning, 2-finger pinch zooming, and Tap-to-Move/Select (ADR 0038). Now utilizes `MapUtils` for squad spawn validation during the deployment phase. Supports native HTML5 drag and drop for unit deployment with custom circular drag images, and manual canvas-based dragging during deployment with closest-unit selection and visual ghost feedback. **Fix:** Refactored unit selection during deployment to use `findLast`, ensuring the top-most unit is correctly picked when multiple units overlap in the same cell. Implements `grab` and `grabbing` cursors for better tactical feedback.
...
- `screens/`: Individual screen components for the Strategic and Tactical layers.
  ...
  - `EquipmentScreen.ts`: Handles soldier loadouts and armory. Implements pay-to-equip economic logic where new equipment purchases deduct Scrap from the campaign balance while re-equipping owned items is free. Now correctly displays "Leave Shop" instead of "Back" when in Shop mode and suppresses mission launch.
