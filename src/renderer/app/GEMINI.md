...
- `InputOrchestrator.ts`: Consolidates tactical input handling (pan/zoom/unit selection). It coordinates between the InputManager, GameClient, and MissionRunner to handle tactical interactions. Now utilizes `MapUtils` for squad spawn validation during the deployment phase.
