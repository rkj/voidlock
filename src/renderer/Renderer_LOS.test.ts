...
// Mock Image
vi.stubGlobal('Image', class { src = ''; onload = null; });
...
describe('Renderer LOS', () => {
  ...
  beforeEach(() => {
    ...
    state = {
        t: 0,
        map,
        units: [{ id: 'u1', pos: { x: 5, y: 5 }, hp: 100, maxHp: 100, state: UnitState.Idle, sightRange: 5, speed: 1, attackRange: 5, damage: 10, fireRate: 100, commandQueue: [] }],
        enemies: [{ id: 'e1', pos: { x: 6, y: 6 }, hp: 50, maxHp: 50, speed: 1, attackRange: 1, damage: 10, fireRate: 100, type: 'Melee' }],
        visibleCells: ['6,6'],
        discoveredCells: [],
        objectives: [],
  ...
  it('should create radial gradient for units when losOverlayEnabled is true', () => {
    ...
    expect(mockContext.createRadialGradient).toHaveBeenCalledWith(160, 160, 0, 160, 160, 160);
  ...
  it('should create radial gradient for visible enemies', () => {
    ...
    expect(mockContext.createRadialGradient).toHaveBeenCalledWith(192, 192, 0, 192, 192, 320);
  });
});
