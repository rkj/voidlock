...
- `CampaignScreen.ts`: Visualizes the Sector Map (DAG) for the campaign mode. Allows navigation between nodes and mission selection. Includes a "New Campaign" wizard with card-based difficulty selection, tactical pause options, and campaign length selection (Standard vs Extended) if no active campaign exists.
