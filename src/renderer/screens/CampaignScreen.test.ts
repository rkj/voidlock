...
describe("CampaignScreen", () => {
  ...
  it("should render 'No Active Campaign' when state is null", () => {
    const screen = new CampaignScreen("screen-campaign", manager, onNodeSelect, onBack);
    screen.show();
  ...
  it("should trigger onNodeSelect when an accessible node is clicked", () => {
    ...
    const accessibleNode = container.querySelector(".campaign-node.accessible") as HTMLElement;
  ...
  it("should trigger onBack when back button is clicked", () => {
    ...
    const backBtn = Array.from(container.querySelectorAll("button")).find(btn => btn.textContent === "BACK TO MENU");
    backBtn?.click();
