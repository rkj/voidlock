import { CommandType, EngagementPolicy, GameState, OverlayOption, UnitState, Vector2 } from '../shared/types';

export type MenuState = 'ACTION_SELECT' | 'TARGET_SELECT' | 'UNIT_SELECT' | 'MODE_SELECT';
...
export class MenuController {
    ...
    public handleCanvasClick(cell: Vector2, gameState: GameState): void {
        if (this.menuState === 'TARGET_SELECT' && (this.pendingAction === CommandType.MOVE_TO)) {
    ...
    public goBack() {
        if (this.menuState === 'UNIT_SELECT') {
            if (this.pendingAction === CommandType.SET_ENGAGEMENT) this.menuState = 'MODE_SELECT';
            else if (this.pendingAction === CommandType.MOVE_TO) this.menuState = 'TARGET_SELECT';
            else this.menuState = 'ACTION_SELECT';
        } else if (this.menuState === 'MODE_SELECT' || this.menuState === 'TARGET_SELECT') {
            this.menuState = 'ACTION_SELECT';
        }
    ...
    private executePendingCommand(unitIds: string[]) {
        ...
        if (this.pendingAction === CommandType.MOVE_TO && this.pendingTargetLocation) {
        ...
        if (type === 'ITEM') {
            gameState.objectives.forEach(obj => {
                if (obj.state === 'Pending' && obj.visible && obj.targetCell) {
                    this.overlayOptions.push({ key: counter.toString(), label: `Collect ${obj.kind}`, pos: obj.targetCell });
                    counter++;
                }
            });
        } else if (type === 'CELL') {
            if (gameState.map.extraction) {
                 this.overlayOptions.push({ key: counter.toString(), label: 'Extraction', pos: gameState.map.extraction });
                 counter++;
            }
            gameState.objectives.forEach(obj => {
                if (obj.state === 'Pending' && obj.visible && obj.targetCell) {
                    this.overlayOptions.push({ key: counter.toString(), label: `Obj ${obj.id}`, pos: obj.targetCell });
                    counter++;
                }
            });
        }
