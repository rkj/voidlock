...
- `units.ts`: Unit definitions and behaviors (`Unit`, `UnitState`, `Archetype`, `Enemy`, `Command`). Now includes the `AVOID` engagement policy for tactical kiting behaviors (ADR 0023). `IGNORE` mode allows firing while moving without engaging in tactical positioning. `AVOID` mode enables tactical kiting while maintaining line-of-sight. Includes `AIPlan` for plan-then-execute commitment (ADR 0056).
