...
describe("TreeShipGenerator Golden Regression", () => {
  it("should match the golden map (JSON and ASCII) for seed 785411", () => {
    ...
    // 1. JSON Comparison
    const jsonGoldenPath = path.join(__dirname, "../../../data/goldens/broken_map_golden.json");
    if (fs.existsSync(jsonGoldenPath)) {
      ...
      const expectedMap = goldenData.replayData ? goldenData.replayData.map : goldenData;

      expect(generatedMap.width).toBe(expectedMap.width);
      expect(generatedMap.height).toBe(expectedMap.height);
      
      const sortCells = (cells: any[]) => [...cells].sort((a, b) => (a.y - b.y) || (a.x - b.x));
    ...
    // 2. ASCII Comparison
    const asciiGoldenPath = path.join(__dirname, "../../../data/goldens/broken_map_golden.txt");
