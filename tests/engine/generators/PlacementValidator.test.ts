...
describe("PlacementValidator", () => {
  ...
  it("should allow occupying an empty cell", () => {
    const validator = new PlacementValidator();
    const pos = { x: 5, y: 5 };
    const success = validator.occupy(pos, OccupantType.SquadSpawn, "room-1");
  ...
  it("should reject occupying an already occupied cell", () => {
    const validator = new PlacementValidator();
    const pos = { x: 5, y: 5 };
    validator.occupy(pos, OccupantType.SquadSpawn, "room-1");

    const success = validator.occupy(pos, OccupantType.EnemySpawn, "room-1");
  ...
  it("should allow occupying different cells", () => {
    const validator = new PlacementValidator();
    validator.occupy({ x: 1, y: 1 }, OccupantType.SquadSpawn, "room-1");
    validator.occupy({ x: 2, y: 2 }, OccupantType.EnemySpawn, "room-2");
    validator.occupy({ x: 3, y: 3 }, OccupantType.Extraction, "room-3");
    validator.occupy({ x: 4, y: 4 }, OccupantType.Objective, "room-4");
