...
describe("PlacementValidator - Loot in Corridors", () => {
  it("should fail validation if bonus loot is in a corridor", () => {
    const goldenPath = path.join(process.cwd(), "tests/data/goldens/2026-01-13-7x7_item_in_corridor.json");
    ...
    expect(result.isValid, "Map should be invalid due to bonus loot in corridor").toBe(false);
    expect(result.issues.some(issue => issue.includes("Loot at (1, 2) must be in a room, not a corridor"))).toBe(true);
  });
  ...
  it("should fail validation if an objective is in a corridor", () => {
    ...
    expect(result.isValid, "Map should be invalid due to objective in corridor").toBe(false);
    expect(result.issues.some(issue => issue.includes("Objective elite-0 at (1, 2) must be in a room, not a corridor"))).toBe(true);
  });
  ...
  it("should fail validation if bonus loot overlaps with squad spawn", () => {
    ...
    const result = MapValidator.validate(map);
    expect(result.isValid, "Map should be invalid due to loot overlapping with squad spawn").toBe(false);
    expect(result.issues.some(issue => issue.includes("overlaps with Squad spawn"))).toBe(true);
  });
});

import { PlacementValidator, OccupantType } from "@src/engine/generators/PlacementValidator";
...
describe("PlacementValidator - Corridor Ban Enforcement", () => {
  ...
  it("should refuse to occupy a cell if it is a corridor", () => {
    const validator = new PlacementValidator();
    const result = validator.occupy({ x: 1, y: 1 }, OccupantType.Loot, "corridor-1");
    expect(result).toBe(false);
  });

  it("should allow occupying a cell if it is a room", () => {
    const validator = new PlacementValidator();
    const result = validator.occupy({ x: 1, y: 1 }, OccupantType.Loot, "room-1");
