...
import { MapDefinition, CellType, MissionType, GameState, CampaignNodeType } from "@src/shared/types";
...
describe("Regression voidlock-uvkz: Mission objective overlap", () => {
  it("should not place dynamic objectives on top of enemy spawn points", () => {
    ...
    for (let s = 0; s < 100; s++) {
        ...
        if (st.objectives.some(o => o.targetCell?.x === 5 && o.targetCell?.y === 5)) {
            overlapFound = true;
            break;
        }
    }

    expect(overlapFound, "Dynamic objective should not overlap with EnemySpawn even across many runs").toBe(false);
  });
});
