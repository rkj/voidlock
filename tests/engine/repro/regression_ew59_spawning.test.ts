...
describe("Regression voidlock-ew59: Point-Based Spawning", () => {
  ...
  it("should verify pre-spawning logic (Rooms only, Point Budget, Safety Quadrant)", () => {
    ...
    spawnedEnemies.forEach(enemy => {
        ...
        expect(cell).toBeDefined();
        expect(cell?.roomId, `Enemy at (${ex}, ${ey}) has no roomId`).toBeDefined();
        expect(cell?.roomId?.startsWith("room-"), `Enemy at (${ex}, ${ey}) is in roomId ${cell?.roomId} (not a room)`).toBe(true);
        ...
        map.squadSpawns?.forEach(ss => {
            ...
            expect(enemyQuad, `Enemy at (${ex}, ${ey}) in quad ${enemyQuad} same as squad spawn quad ${squadQuad}`).not.toBe(squadQuad);
        });
        if (map.squadSpawn) {
