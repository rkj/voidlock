...
describe("AI Target Stability", () => {
  ...
  it("Soldier should maintain focus on one target and not oscillate movement", () => {
    ...
    const internalEnemy = (engine as any).state.enemies.find(
      (e: Enemy) => e.id === otherEnemyId,
    )!;
    if (otherEnemyId === "e1") {
  ...
  it("Enemies should stick to their initial target for a minimum duration", () => {
    ...
    const internalOtherSoldier = (engine as any).state.units.find(
      (u: Unit) => u.id === otherSoldierId,
    )!;
