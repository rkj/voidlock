...
describe("MenuConfig New Commands", () => {
  ...
  beforeEach(() => {
    ...
    mockState = createMockGameState({
      ...
      units: [
        { id: "u1", pos: { x: 0.5, y: 0.5 }, state: UnitState.Idle, stats: { speed: 20 } } as any,
        { id: "u2", pos: { x: 0.5, y: 0.5 }, state: UnitState.Idle, stats: { speed: 20 } } as any
      ],
      enemies: [],
  ...
  it("should have PICKUP in ACTION_SELECT", () => {
    ...
    const pickupOption = renderState.options.find(o => o.label.includes("PICKUP"));
  ...
  it("should have EXTRACT in ACTION_SELECT", () => {
    const renderState = controller.getRenderableState(mockState);
    const extractOption = renderState.options.find(o => o.label.includes("EXTRACT"));
  ...
  it("should have ESCORT in ORDERS_SELECT", () => {
    ...
    const renderState = controller.getRenderableState(mockState);
    const escortOption = renderState.options.find(o => o.label.includes("ESCORT"));
  ...
  it("should have HOLD as key 5 in ORDERS_SELECT", () => {
    controller.handleMenuInput("1", mockState); // ORDERS
    const renderState = controller.getRenderableState(mockState);
    const holdOption = renderState.options.find(o => o.label.includes("HOLD"));
  ...
  it("should show loot in TARGET_SELECT for PICKUP", () => {
    ...
    const renderState = controller.getRenderableState(mockState);
    const lootOption = renderState.options.find(o => o.label.includes("Pickup medkit"));
