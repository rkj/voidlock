...
describe("CampaignShell", () => {
  ...
  beforeEach(() => {
    ...
    shell = new CampaignShell("screen-campaign-shell", manager as any, onTabChange, onMenu);
  });
  ...
  it("should call onTabChange when a tab is clicked", () => {
    ...
    const barracksBtn = Array.from(container.querySelectorAll("button")).find(b => b.textContent === "Barracks");
  ...
  it("should call onMenu when Main Menu button is clicked", () => {
    ...
    const menuBtn = Array.from(container.querySelectorAll("button")).find(b => b.textContent === "Main Menu");
