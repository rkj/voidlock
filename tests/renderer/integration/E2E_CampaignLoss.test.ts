...
describe("E2E Campaign Failure Modes", () => {
  ...
  it("Ironman: Start Ironman -> Force Lose -> Verify Defeat", async () => {
    ...
    stateUpdateCallback!(createMockState({
      status: "Lost",
      t: 100,
      stats: { aliensKilled: 0, elitesKilled: 0, scrapGained: 0, threatLevel: 0, casualties: 4 },
      units: cm.getState()!.roster.map(s => ({
        id: s.id,
        hp: 0, // All dead
        maxHp: s.maxHp,
  ...
  it("Standard: Start Standard -> Kill Soldier -> Verify DEAD in Roster -> Not in next squad", async () => {
    ...
    const deadSoldier = cm.getState()!.roster.find(s => s.id === deadSoldierId);
    ...
    // Check squad builder
    const rosterCards = document.querySelectorAll("#squad-builder .soldier-card");
    ...
    const deadInSquadAfterClick = Array.from(activeSquadSlotsAfterClick).some(slot => slot.querySelector("strong")?.textContent?.includes("Recruit 1"));
