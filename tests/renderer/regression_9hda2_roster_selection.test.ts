...
describe("Regression: Campaign Roster Selection (voidlock-9hda.2)", () => {
  ...
  function renderSquadBuilder(isCampaign: boolean = false) {
    ...
    if (isCampaign) {
      ...
      state.roster.forEach((soldier) => {
        ...
        const isChecked = currentSquad.soldiers.some(
          (s: any) => s.id === soldier.id,
        );
  ...
  it("should disable checkboxes for non-healthy soldiers", () => {
    renderSquadBuilder(true);
    const s3Checkbox = document.querySelector(
      'input[data-id="s3"]',
    ) as HTMLInputElement;
  ...
  it("should add soldier to squad when checked", () => {
    renderSquadBuilder(true);
    const s1Checkbox = document.querySelector(
      'input[data-id="s1"]',
    ) as HTMLInputElement;
  ...
  it("should enforce max squad size in campaign mode", async () => {
    ...
    renderSquadBuilder(true);
    const s6Checkbox = document.querySelector(
      'input[data-id="s6"]',
    ) as HTMLInputElement;
