...
describe("MapValidator", () => {
  it("should validate a correct map definition", () => {
    const validMap = {
      ...
      extraction: { x: 9, y: 9 },
      objectives: [
        { id: "obj1", kind: "Recover", targetCell: { x: 5, y: 5 } }
      ],
      spawnPoints: [
        { id: "sp1", pos: { x: 2, y: 2 }, radius: 1 }
      ],
      walls: [
  ...
  it("should fail if width or height are missing or out of bounds", () => {
    ...
    expect(MapValidator.validateMapData({ width: 0, height: 10, cells: [] })).toBe(false);
    expect(MapValidator.validateMapData({ width: 101, height: 10, cells: [] })).toBe(false);
  });

  it("should fail if cells are missing or invalid", () => {
    expect(MapValidator.validateMapData({ width: 10, height: 10 })).toBe(false);
    expect(MapValidator.validateMapData({ width: 10, height: 10, cells: {} })).toBe(false);
    expect(MapValidator.validateMapData({
      width: 10,
      height: 10,
