...
describe("Custom Mission Scrollbar Clipping Repro", () => {
  ...
  it("should ensure the 'Confirm' button is reachable on small viewports", async () => {
    ...
    await page.screenshot({
      path: "tests/e2e/__snapshots__/scrollbar_clipping_verification.png",
    });
    ...
    expect(
      conditionMet,
      "The button should either be in viewport, or the container should be scrollable to reach it",
    ).toBe(true);
    ...
    if (!isButtonInViewport) {
      expect(
        isScrollable,
        "If button is off-screen, the container MUST be scrollable",
      ).toBe(true);
    }
  });
});
