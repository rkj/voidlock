...
describe('Tap-to-Inspect E2E', () => {
  ...
  it('should show popover on stat icon tap', async () => {
    ...
    try {
      await page.waitForSelector('.stat-display[data-tooltip]', { timeout: 15000 });
    } catch (e) {
    ...
    // Tap on the icon
    await page.touchscreen.tap(iconBoundingBox!.x + iconBoundingBox!.width / 2, iconBoundingBox!.y + iconBoundingBox!.height / 2);
    ...
    const popoverVisible = await page.evaluate(() => {
      ...
      return popover !== null && window.getComputedStyle(popover).opacity !== '0';
    });

    expect(popoverVisible).toBe(true);

    // Take screenshot for manual verification if needed
    await page.screenshot({ path: 'screenshots/tap_to_inspect_verification.png' });

    // Tap outside to dismiss
    await page.touchscreen.tap(10, 10);
    const popoverGone = await page.evaluate(() => document.querySelector('.inspect-popover') === null);
