...
describe("Global Error Alert & Reset E2E", () => {
  ...
  it("should show confirm dialog on global error and reset data on OK", async () => {
    ...
    await page.evaluate(() => {
      ...
      script.textContent =
        'setTimeout(() => { throw new Error("Test Error for Global Alert"); }, 100);';
  ...
  it("should show panic UI if confirm is cancelled", async () => {
    ...
    await page.evaluate(() => {
      const script = document.createElement("script");
      script.textContent =
        'setTimeout(() => { throw new Error("Test Error for Panic UI"); }, 100);';
      document.body.appendChild(script);
    });
    ...
    const testData = await page.evaluate(() =>
      localStorage.getItem("test-data-keep"),
    );
  ...
  it("should show confirm dialog on unhandled promise rejection", async () => {
    ...
    await page.evaluate(() => {
      const script = document.createElement("script");
      script.textContent =
        'setTimeout(() => { Promise.reject(new Error("Test Rejection for Global Alert")); }, 100);';
      document.body.appendChild(script);
    });
