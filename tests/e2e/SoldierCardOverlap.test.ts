...
describe("Soldier Card Overlap Verification", () => {
  ...
  it("should verify that the remove 'X' does not overlap with 'LVL' on soldier cards in Equipment Screen (Mobile)", async () => {
    try {
      ...
      await page.waitForFunction(() => document.documentElement.classList.contains('mobile-touch'));
      ...
      await page.evaluate(() => {
        ...
        const setupBtn = buttons.find(b => b.textContent?.includes('Simulation Setup'));
      ...
      const positions = await page.evaluate(() => {
        const item = document.querySelector(".soldier-list-panel .soldier-item");
        if (!item) return null;
        ...
        if (!lvl || !x) return { error: "Elements not found", lvl: !!lvl, x: !!x };
        ...
        return {
          ...
          overlap,
          lvl: { top: lvlRect.top, bottom: lvlRect.bottom, left: lvlRect.left, right: lvlRect.right },
          x: { top: xRect.top, bottom: xRect.bottom, left: xRect.left, right: xRect.right }
        };
      });
      
      console.log("Equipment Positions (Mobile):", JSON.stringify(positions, null, 2));
