  ...
  it("should reproduce layout clipping on the Equipment Screen at small viewports", async () => {
    ...
    await page.evaluate((config) => {
      ...
      squadConfig: {
        soldiers: Array.from({ length: 100 }, (_, i) => ({ 
          id: `soldier-${i}`,
          archetypeId: i % 4 === 0 ? "assault" : i % 4 === 1 ? "medic" : i % 4 === 2 ? "heavy" : "scout",
          hp: 100,
          maxHp: 100,
          soldierAim: 70
        })),
    ...
    const clippingStats = await page.evaluate(() => {
      ...
      return {
        ...
        isClippedByShell: shellRect && footerRect ? footerRect.bottom > shellRect.bottom : false
      };
    });
    ...
    expect(clippingStats.isClippedByShell, "Footer is clipped by shell content").toBe(false);
    
    const allPanelsScrollable = clippingStats.panelStats?.every(p => p.scrollable);
    expect(allPanelsScrollable, "One or more panels are NOT scrollable even with many items").toBe(true);
  });
});
